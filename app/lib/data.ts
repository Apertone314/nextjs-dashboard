...
export async function getData() : Promise<Project[]>{
  try {
    ...
    const [rows] = await connection.query<RowDataPacket[]>('SELECT pkey_id,latitude,longitude,power_kwc,type_subsidy FROM projects');
...
export async function fetchClients() : Promise<Client[]>{
  try {
    // Exécuter une requête SQL pour récupérer les données
    const [rows] = await connection.query<RowDataPacket[]>('SELECT * FROM clients');
